...
public class CodeGenerateUtils {
	...
	public static void main(String[] args) throws Exception {
		...
		codeGenerateUtils.generateFile("qd_role", "演示");
	}
	...
	public void generateFile(String tableName, String tableAnnotation) throws Exception {
		try {
			...
			if (projectPath.indexOf(moduleName) != -1) {
				targetPath = projectPath + "/src/main/java/" + StringUtils.join(packageArr, "/");
			} else {
			...
			ResultSet resultSet = databaseMetaData.getColumns(null, "%", tableName.toUpperCase(), "%");
			ResultSet pkSet = databaseMetaData.getPrimaryKeys(null, null, tableName.toUpperCase());
			// 获取数据表列信息
			Map<String, Object> dataMap = getColumnsList(resultSet, pkSet);
	 ...
	 * @throws IOException
	 */
	private Map<String, Object> getColumnsList(ResultSet resultSet, ResultSet pkSet) throws IOException {
		// 初始化Map对象
		Map<String, Object> dataMap = new HashMap<>();
		try {
			// 获取主键信息
			...
			while (pkSet.next()) {
				pkBuffer.append(pkSet.getString("COLUMN_NAME").toLowerCase()).append(",");
			}
			// 获取列信息
			...
			while (resultSet.next()) {
				...
				if (pkBuffer.toString().indexOf(columnName) != -1) {
					columnClass.setIsPk(1);
				} else {
					columnClass.setIsPk(0);
				}

...
public class UploadUtils {
	...
	public Map<String, Object> uploadFile(HttpServletRequest request, String uploadPath, String name) {
		// 验证文件并返回错误信息
		String error = this.validateFields(request, uploadPath, name);
		// 初始化表单元素
		Map<String, Object> fieldsMap = new HashMap<String, Object>();
		if (error.equals("")) {
			fieldsMap = this.initFields(request, uploadPath);
		}
		List<FileItem> fiList = (List<FileItem>) fieldsMap.get(UploadUtils.FILE_FIELDS);
		if (fiList != null) {
			for (FileItem item : fiList) {
				// 上传文件并返回错误信息
				error = this.saveFile(uploadPath, item);
			}
		}
		// 返回结果
	 ...
	 * @return
	 */
	private String validateFields(HttpServletRequest request, String uploadPath, String name) {
		String errorInfo = "";
		// 获取内容类型
		...
		if (contentType == null || !contentType.startsWith("multipart")) {
			...
			// 创建上传临时目录
			File file = new File(uploadPath + TEMP_PATH);
			if (!file.exists()) {
				file.mkdirs();
			}
		}
	 ...
	 */
//	@SuppressWarnings("unchecked")
	private Map<String, Object> initFields(HttpServletRequest request, String uploadPath) {
		// 存储表单字段和非表单字段
		...
		if (isMultipart) {
			...
			// 设置上传文件的临时目录
			factory.setRepository(new File(uploadPath + TEMP_PATH));
			// 核心操作类:创建一个文件上传解析器。
	 ...
	 * @return
	 */
	private String saveFile(String uploadPath, FileItem item) {
		String error = "";
		...
		if (item.getSize() > maxSize) { // 检查文件大小
			...
			// 写入文件
			try {
				File uploadedFile = new File(uploadPath + TEMP_PATH, newFileName);
